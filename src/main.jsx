...
const App = () => {
  ...
  const navigate = (key, explicitCounty, slug) => {
    ...
    // navigate('events', null) → hub; navigate('events') → stored county or hub
    if (key === 'events') {
      setPage('events');
      setVenueSlug(null);
      if (explicitCounty) {
  ...
  switch (displayPage) {
    ...
    case 'find-help': content = <React.Suspense key={county || 'all'} fallback={<RouteLoading />}><FindHelpPage onNavigate={navigate} session={session} county={county} /></React.Suspense>; break;
