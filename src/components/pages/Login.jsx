// Sign-in page for organisation owners and administrators.
// Password reset completion lives at /reset-password (ResetPassword.jsx).
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (data?.session) {
        ...
        const nextRoute = ADMIN_EMAIL_ALLOWLIST.includes(signedInEmail) ? 'admin' : 'profile';
      ...
      <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24, background: 'linear-gradient(180deg, #F8FBFF 0%, #FFFFFF 100%)' }}>
        <div className="card" style={{ width: '100%', maxWidth: 480, padding: 40, borderRadius: 32 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 }}>
            <div style={{ width: 64, height: 64, borderRadius: 20, background: 'rgba(45,156,219,0.12)', display: 'grid', placeItems: 'center', color: '#2D9CDB' }}>
              <IHub s={32} />
            </div>
            <div>
          ...
          {!isSupabaseConfigured() && (
            <div style={{ borderRadius: 18, border: '1px solid rgba(244,97,58,0.28)', background: 'rgba(244,97,58,0.05)', padding: 18, marginBottom: 24 }}>
              <div style={{ fontWeight: 700, color: '#A03A2D', marginBottom: 8 }}>Supabase not configured</div>
              <div style={{ color: 'rgba(26,39,68,0.7)', fontSize: 14 }}>Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to your environment variables.</div>
            </div>
          )}

          <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 16, marginBottom: 24 }}>
            <div>
              <label style={{ display: 'block', fontSize: 14, fontWeight: 600, marginBottom: 8, color: '#1A2744' }}>Email</label>
              <input
                type="email"
                value={email}
            ...
            {successMsg && (
              <div style={{ borderRadius: 14, border: '1px solid rgba(91,201,74,0.28)', background: 'rgba(91,201,74,0.05)', padding: 14, color: '#2D6B1F', fontSize: 14, fontWeight: 600 }}>
                {successMsg}
              </div>
            )}
          ...
          <button
            ...
            style={{ width: '100%', marginTop: 16, padding: '13px 14px', borderRadius: 16, border: '1px solid #EFF1F7', background: '#FAFBFF', color: '#1A2744', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all .15s' }}
            onMouseEnter={(e) => { e.currentTarget.style.background = '#EFF1F7'; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = '#FAFBFF'; }}
          >
            Back to home <IArrow s={14} />
          </button>
