// Groups — live Supabase data from venues_public view.
// Shows category = 'Groups' for selected county.
// Carer support groups, social circles and peer-support activities.
...
const GROUPS_ACCENT = '#2D9CDB'; // blue — social, community
...
const GroupCard = ({ venue, onClaim, onViewProfile }) => {
  const accent = GROUPS_ACCENT;

  const tags = [];
  if (venue.free_or_paid)      tags.push({ label: venue.free_or_paid,  color: venue.free_or_paid === 'Free' ? '#0D7A55' : '#1A2744' });
  if (venue.indoor_outdoor)    tags.push({ label: venue.indoor_outdoor, color: '#7B5CF5' });
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', flex: 1, gap: 7 }}>
        ...
        <div style={{ marginTop: 8, paddingTop: 9, borderTop: '1px solid #EEF4FC', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11.5, color: 'rgba(26,39,68,0.40)' }}>Run this group?</span>
          <button
...
const SkeletonCard = () => (
  <div className="card" style={{ padding: 16, borderRadius: 16, minHeight: 180 }}>
    ...
    <div style={{ height: 10, width: '75%', borderRadius: 6, background: '#EAF0FA' }} />
  </div>
);

// ── Page component ─────────────────────────────────────────────────────────

const GroupsPage = ({ onNavigate, session, county, venueSlug }) => {
  if (venueSlug) {
    return (
      <VenueProfile
        slug={venueSlug}
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) { setError(err.message || 'Could not load groups.'); setLoading(false); }
      }
    };
  ...
  return (
    <>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(150deg, #0A1828 0%, #0E2340 50%, #122B4D 100%)', paddingTop: 36, paddingBottom: 36 }}>
        <div style={{ position: 'absolute', top: -60, right: -60, width: 380, height: 380, borderRadius: '50%', background: 'radial-gradient(circle, rgba(45,156,219,0.14) 0%, transparent 65%)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: -60, left: '25%', width: 340, height: 340, borderRadius: '50%', background: 'radial-gradient(circle, rgba(96,195,240,0.08) 0%, transparent 65%)', pointerEvents: 'none' }} />

        <div className="container" style={{ position: 'relative' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px', borderRadius: 999, background: 'rgba(45,156,219,0.20)', border: '1px solid rgba(45,156,219,0.32)', fontSize: 11, fontWeight: 800, color: '#7DD3F8', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 12 }}>
            Groups & Social · {countyLabel}
          </div>
          <h1 style={{ fontSize: 'clamp(22px, 4vw, 40px)', fontWeight: 800, color: '#FFFFFF', letterSpacing: '-0.03em', lineHeight: 1.08, margin: '0 0 10px', textWrap: 'balance' }}>
            Carer Groups in {countyLabel}
          </h1>
          <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.68)', lineHeight: 1.6, margin: '0 0 16px', maxWidth: 520 }}>
            Find local support groups, social circles, peer-support meetups and community activities for carers across {countyLabel}.
            {!loading && venues.length > 0 && (
              <> Browse <strong style={{ color: '#FFFFFF' }}>{venues.length} groups</strong> near you.</>
            )}
          </p>
          ...
          {!loading && venues.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 0 }}>
              {[
                { n: venues.length,                                           l: 'Groups listed' },
                { n: venues.filter((v) => v.free_or_paid === 'Free').length,  l: 'Free to attend' },
      ...
      <section style={{ background: '#FFFFFF', borderBottom: '1px solid #EEF1F7', paddingTop: 12, paddingBottom: 12, position: 'sticky', top: 72, zIndex: 40 }}>
        <div className="container">
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, alignItems: 'center' }}>
            ...
            <button onClick={() => setFilterCarer(!filterCarer)}           style={chip(GROUPS_ACCENT, filterCarer)}>Carer friendly</button>
      ...
      {/* ── Main content ──────────────────────────────────────────────── */}
      <section style={{ paddingTop: 32, paddingBottom: 56, background: '#F7F9FC', minHeight: '50vh' }}>
        <div className="container">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 8 }}>
              <div style={{ fontSize: 13.5, color: 'rgba(26,39,68,0.55)' }}>
                {filtered.length === venues.length
                  ? <><strong style={{ color: '#1A2744' }}>{venues.length}</strong> groups in {countyLabel}</>
                  : <><strong style={{ color: '#1A2744' }}>{filtered.length}</strong> of {venues.length} groups</>
                }
              </div>
          ...
          {!loading && error && (
            <div style={{ textAlign: 'center', padding: '64px 20px' }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: '#1A2744', marginBottom: 6 }}>Could not load groups</div>
          ...
          {!loading && !error && filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '64px 20px' }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 700, color: '#1A2744', marginBottom: 6 }}>
                {venues.length === 0 ? 'No groups listed yet' : 'No groups match your filters'}
              </div>
              <div style={{ fontSize: 13.5, color: 'rgba(26,39,68,0.55)', marginBottom: 18, maxWidth: 360, margin: '0 auto 18px' }}>
                {venues.length === 0
                  ? `We're adding carer groups in ${countyLabel} — check back soon.`
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
              {filtered.map((venue) => (
                <GroupCard
                  ...
                  onViewProfile={(slug) => onNavigate('groups', county || 'cornwall', slug)}
                />
              ))}
            </div>
          )}
